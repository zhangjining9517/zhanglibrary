...
public class CalendarView extends View implements View.OnTouchListener {
	...
	private void init() {
		curDate = selectedStartDate = selectedEndDate = today = setDate(new Date());
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		surface.height = (int) (getResources().getDisplayMetrics().heightPixels * 4 / 5);
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < 42; i++) {
			...
			int not_current_bg_color = surface.not_current_month_bg;
			if (isLastMonth(i)) {
				color = surface.not_current_month_textColor;
				drawCellBg(canvas, i, not_current_bg_color);
	...
	private void drawCellText(Canvas canvas, int index, String text, int color) {
		...
		float cellY = surface.monthHeight + surface.weekHeight + (y - 1)
				* surface.cellHeight + surface.cellHeight * 2 / 4f;
		...
		case MotionEvent.ACTION_UP:
			if (downDate != null) {
				...
				onItemClickListener.OnItemClick(
						m_Format.format(selectedStartDate), false);
				downDate = null;
				invalidate();
			}
			break;
	...
	public interface OnItemClickListener {
		void OnItemClick(String strDate, boolean isData);
	}
	...
	private class Surface {
		...
		// 设置上一个月与下一个月的背景颜色
		private int not_current_month_bg = Color.parseColor("#EEEEEE");
		// 当前月、周的字体的颜色
		private int textColor = Color.BLACK;
		// 显示查询字体的颜色
		private int query_textColor = Color.GREEN;
		// 非当前月、周的字体的颜色
		private int not_current_month_textColor = Color.parseColor("#CDC5BF");
		// 周的字体的颜色
		private int week_textColor = Color.parseColor("#AAAAAA");
		// 画边框的颜色
		private int borderColor = Color.parseColor("#D7D7D7");
		// 画有数据的天的颜色背景
		private int jichedata_bg = Color.parseColor("#76EE00");
		public int todayNumberColor = Color.RED;
		public int cellDownColor = Color.parseColor("#FFCF52");
		// public int cellSelectedColor = Color.parseColor("#5CACEE");
		...
		// 显示查询数据的画笔
		public Paint queryPaint;
		public Paint cellBgPaint;
		public Paint noCurrentMonthPaint;
		...
		public void init() {
			...
			// 画最上面的每周的画笔
			weekPaint = new Paint();
			weekPaint.setColor(week_textColor);
			weekPaint.setAntiAlias(true);
			// float weekTextSize = weekHeight * 0.6f;
			// Log.d(TAG, "week size:" + weekTextSize);
			weekPaint.setTextSize(30.0f);
			weekPaint.setTypeface(Typeface.DEFAULT_BOLD);
			// 画最上面的每周的画笔
			noCurrentMonthPaint = new Paint();
			noCurrentMonthPaint.setColor(not_current_month_textColor);
			noCurrentMonthPaint.setAntiAlias(true);
			// float weekTextSize = weekHeight * 0.6f;
			// Log.d(TAG, "week size:" + weekTextSize);
			noCurrentMonthPaint.setTextSize(30.0f);
			noCurrentMonthPaint.setTypeface(Typeface.DEFAULT_BOLD);
			// //
			datePaint = new Paint();
			datePaint.setColor(textColor);
			datePaint.setAntiAlias(true);
			// float cellTextSize = cellHeight * 0.5f;
			// Log.d(TAG, "date size:" + cellTextSize);
			datePaint.setTextSize(30.0f);
			datePaint.setTypeface(Typeface.DEFAULT_BOLD);
			queryPaint = new Paint();
			queryPaint.setColor(query_textColor);
			queryPaint.setAntiAlias(true);
			// float cellTextSize = cellHeight * 0.5f;
			// Log.d(TAG, "date size:" + cellTextSize);
			queryPaint.setTextSize(20.0f);
			queryPaint.setTypeface(Typeface.DEFAULT_BOLD);
